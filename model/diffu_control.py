...
from .build import load_sd_model, load_clip_model_MetaCLIP
...
class SDModel(PreTrainedModel):
    def __init__(
        ...
        self.model_id, self.pipe, self.vae, self.tokenizer, self.text_encoder, \
                self.unet, self.scheduler, self.image_renormalizer = load_sd_model(config)
        ...
        self.class_model = load_clip_model_MetaCLIP(config)
    ...
    def zeroshot_classifier_MetaCLIP(self, classnames, templates, model):
        with torch.no_grad():
            zeroshot_weights = []
    ...
    def forward(
        ...
        with torch.no_grad():
            imagenet_templates = ['{}',]
            zeroshot_weights = self.zeroshot_classifier_MetaCLIP(text, imagenet_templates, self.class_model.model.float()).float()
        ...
        for step in range(self.config.tta.gradient_descent.train_steps):
            ...
            image_features = self.visual_proj(image_features) # b,1024
