...
class SDFeaturizer:
    ...
    def forward_list(self,
        ...
        unet_ft_all = self.pipe(
            ...
            up_ft_indices=up_ft_index,
            prompt_embeds=prompt_embeds)
        unet_ft_all = unet_ft_all['up_ft'] # b, c, h, w
        return unet_ft_all 

    @torch.no_grad()
    def forward_condition_list(self,
                img_tensor,
                prompt='',
                t=261, up_ft_index=[1,2,3], condition_fea=None):
        # img_tensor b,c,h,w
        if prompt == self.null_prompt:
            prompt_embeds = self.null_prompt_embeds
        else:
            prompt_embeds = self.pipe._encode_prompt(
                ...
                # device='cuda:0',
                num_images_per_prompt=1,
                do_classifier_free_guidance=False) # [b, 77, 1024]
        if condition_fea is not None:
            prompt_embeds = prompt_embeds + condition_fea.unsqueeze(1) # [b, 77, 1024] + [b, 1, 1024] = [b, 77, 1024]
        # print("=> img_tensor shape ", img_tensor.shape)
        # print("=> prompt_embeds shape ", prompt_embeds.shape)
        unet_ft_all = self.pipe(
            img_tensor=img_tensor,
